...
void OSGRenderer::wheelEvent(QWheelEvent* event)
{
    ...
    // Mouse motion with scaled position
    m_osgWinEmb->getEventQueue()->mouseMotion(pos.x() * m_windowScale, pos.y() * m_windowScale);

    // Determine the scroll direction
    if (event->angleDelta().y() != 0) {
        // Vertical scrolling
        if (event->angleDelta().y() > 0) {
            m_osgWinEmb->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_UP);
        } else {
            m_osgWinEmb->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_DOWN);
        }
    } else if (event->angleDelta().x() != 0) {
        // Horizontal scrolling
        if (event->angleDelta().x() > 0) {
            m_osgWinEmb->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_LEFT);
        } else {
            m_osgWinEmb->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_RIGHT);
        }
    }
}
